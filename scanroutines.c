...
#define DEFINE_INTEGER_RANGE_ROUTINE(DATATYPENAME, DATAWIDTH) \
    ...
    { \
        int ret = 0; \
        if ((new_value->flags.s##DATAWIDTH##b) && (user_value[0].flags.s##DATAWIDTH##b) && \
           (user_value[1].flags.s##DATAWIDTH##b) && (get_s##DATAWIDTH##b(new_value)>=get_s##DATAWIDTH##b(&user_value[0])) && \
           (get_s##DATAWIDTH##b(new_value)<=get_s##DATAWIDTH##b(&user_value[1])) ) { \
            ret = (DATAWIDTH)/8; SET_FLAG(saveflags, s##DATAWIDTH##b); \
        } \
        if ((new_value->flags.u##DATAWIDTH##b) && (user_value[0].flags.u##DATAWIDTH##b) && \
           (user_value[1].flags.u##DATAWIDTH##b) && (get_u##DATAWIDTH##b(new_value)>=get_u##DATAWIDTH##b(&user_value[0])) && \
...
#define DEFINE_FLOAT_RANGE_ROUTINE(DATATYPENAME, DATAWIDTH) \
    ...
    { \
        int ret = 0; \
        if ((new_value->flags.f##DATAWIDTH##b) && (user_value[0].flags.f##DATAWIDTH##b) && \
           (user_value[1].flags.f##DATAWIDTH##b) && (get_f##DATAWIDTH##b(new_value)>=get_f##DATAWIDTH##b(&user_value[0])) && \
